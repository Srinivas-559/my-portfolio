...
icon?: React.ReactNode;
    ...
    const handleCopy = () =>{
        navigator.clipboard.writeText("srinvasgollapalli597@gmail.com");
        setCopied(true);
    }
      ...
      className={cn(
        ...
        className
          ...
          style={{
            ...
            backgroundColor: 'linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(115,135,175,1) 100%, rgba(0,212,255,1) 100%)'
          }}
      >
          <div className={`${id === 6 && ' flex  justify-center  ' } h-full `}>
              ...
              {id === 2 && <GlobeDemo />}
              {id === 3 &&
                  <div className="flex gap-1  w-fit mt-6  absolute -right-3 lg:-right-2">
                      <div className='flex flex-col  gap-2 lg:gap-3'>
                          {['React.js', 'Next.js', 'TypeScript'].map((item) => (
                          <span key={item} className="py-4 lg:py-3  text-xs px-3 mr-0 lg:text-base opacity-50 lg:opacity-80 rounded-lg text-center bg-[#10132E]">
                              {item}
                          </span>
                          ))}
                      ...
                      <div className='flex flex-col  gap-2 lg:gap-3'>
                          <span className="py-4 px-3 rounded-lg text-center bg-[#10132E]"></span>
                          {['Solidity', 'Foundry', 'Ethers.js'].map((item) => (
                          <span key={item} className="py-4 lg:py-3  text-xs px-3 mr-0 lg:text-base opacity-50 lg:opacity-80 rounded-lg text-center bg-[#10132E]">
                              {item}
                          </span>
                          ))}
